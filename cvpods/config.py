...
_config_dict = dict(
    ...
    MODEL=dict(
        WEIGHTS='/1T/zxp/Harmonious-Teacher-yuan/cvpods/pretrained_model/vgg16_wo_bn_caffe_normal.pkl',
    ...
    TRAINER=dict(
        ...
        EMA=dict(
            DECAY_FACTOR=0.9996,
            UPDATE_STEPS=1,
            START_STEPS=3000,  # adjust for 4-GPU setting
            FAKE=False
        ),
        SSL=dict(
            BURN_IN_STEPS=5000,  # adjust for 4-GPU setting
        ),
        DISTILL=dict(
            RATIO=0.01,
            HM=dict(
