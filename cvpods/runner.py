...
class SemiRunner(DefaultRunner):
    ...
    def run_step(self):
        ...
        src_image = sup_weak[0]['image'].cpu().numpy()  # [3,600,1200]
        tgt_image = unsup_weak[0]['image'].cpu().numpy()
        src_in_trg = FDA_source_to_target_np(src_image, tgt_image, L=0.1)  # src_lbl
        sup_weak_fft = sup_weak
        sup_weak_fft[0]['image'] = torch.Tensor(src_in_trg).cuda()
        ...
        loss_dict_sup = self.model(sup_weak, sup=True)
        ...
        loss_dict_sup_fft = self.model(sup_weak_fft, FFT=True)
        loss_dict_sup_fft = {k: v * self.cfg.TRAINER.DISTILL.SUP_WEIGHT for k, v in loss_dict_sup_fft.items()}
        losses_sup_fft = sum([
            metrics_value for metrics_value in loss_dict_sup_fft.values()
            if metrics_value.requires_grad
        ]) * 2



        losses_sup = losses_sup + losses_sup_fft
        losses_sup.backward()
        ...
        if self.iter > self.burn_in_steps:
            ...
            student_logits, student_deltas, student_quality, box_xyxy, losses_dis_GA_t, losses_dis_CA_t = self.model(unsup_strong, get_data=True)

            student_logits_fft, student_deltas_fft, student_quality_fft, box_xyxy_fft, loss_unuse1, loss_unuse2 = self.model(
                sup_weak_fft, get_data=True)

            with torch.no_grad():
                teacher_logits, teacher_deltas, teacher_quality, tea_box_xyxy = self.ema_model.model(unsup_weak, is_teacher=True)

                teacher_logits_fft, teacher_deltas_fft, teacher_quality_fft, tea_box_xyxy_fft = self.ema_model.model(
                    sup_weak_fft,
                    is_teacher=True)
            ...
            loss_dict_unsup['losses_dis_GA_t'] = losses_dis_GA_t
            loss_dict_unsup['losses_dis_CA_t'] = losses_dis_CA_t
            losses_unsup = sum([
            ...
            distill_weights_fft = {
                "distill_loss_logits_fft": self.cfg.TRAINER.DISTILL.WEIGHTS.LOGITS,
                "distill_loss_deltas_fft": self.cfg.TRAINER.DISTILL.WEIGHTS.DELTAS,
                "distill_loss_quality_fft": self.cfg.TRAINER.DISTILL.WEIGHTS.QUALITY,
                "loss_uhl_fft": self.cfg.TRAINER.DISTILL.WEIGHTS.UHL,
                "fore_ground_sum_fft": 1.,
            }
            loss_dict_unsup_fft = self.get_distill_loss(student_logits_fft, student_deltas_fft, student_quality_fft,
                                                        teacher_logits_fft, teacher_deltas_fft, teacher_quality_fft,
                                                        box_xyxy=box_xyxy_fft,
                                                        tea_box_xyxy=tea_box_xyxy_fft, FFT=True)
            loss_dict_unsup_fft = {k: (v * unsup_weight) if v.requires_grad else v for k, v in
                                   loss_dict_unsup_fft.items()}
            loss_dict_unsup_fft = {k: v * distill_weights_fft[k] for k, v in loss_dict_unsup_fft.items()}
            losses_unsup_fft = sum([
                metrics_value for metrics_value in loss_dict_unsup_fft.values()
                if metrics_value.requires_grad
            ]) * 2



            losses_unsup = losses_unsup + losses_unsup_fft
    ...
    def get_distill_loss(self,
                         ...
                         box_xyxy=None, tea_box_xyxy=None, FFT=False):
        ...
        if FFT:
            loss_dict = {
                "distill_loss_logits_fft": loss_logits,
                "distill_loss_quality_fft": loss_quality,
                "distill_loss_deltas_fft": loss_deltas,
                "fore_ground_sum_fft": fg_num,
            }
        if not FFT:
            loss_dict = {
        ...
        if FFT:
            loss_dict.update({'loss_uhl_fft': loss_uhl})
        if not FFT:
...
def FDA_source_to_target_np(src_img, trg_img, L=0.1):
    ...
    # 获取两个数组的shape
    shape1 = amp_src_.shape
    shape2 = pha_src.shape

    max_shape = tuple(max(s1, s2) for s1, s2 in zip(shape1, shape2))

    amp_src_ = np.pad(amp_src_, [(0, max_shape[0] - shape1[0]), (0, max_shape[1] - shape1[1]), (0, max_shape[2] - shape1[2])], mode='constant')
    pha_src = np.pad(pha_src, [(0, max_shape[0] - shape2[0]), (0, max_shape[1] - shape2[1]), (0, max_shape[2] - shape2[2])], mode='constant')
...
def low_freq_mutate_np(amp_src, amp_trg, L=0.1):
    ...
    shape_a_src = a_src.shape
    shape_a_trg = a_trg.shape
    max_shape = tuple(max(s1, s2) for s1, s2 in zip(shape_a_src, shape_a_trg))
    a_src = np.pad(a_src, [(0, max_shape[0] - shape_a_src[0]), (0, max_shape[1] - shape_a_src[1]), (0, max_shape[2] - shape_a_src[2])], mode='constant')
    a_trg = np.pad(a_trg, [(0, max_shape[0] - shape_a_trg[0]), (0, max_shape[1] - shape_a_trg[1]), (0, max_shape[2] - shape_a_trg[2])], mode='constant')
